...
grep -Fq -- '--- PASS: TestLinuxCOREBackendKernelEnforcement' "${LOG}" || {
  echo "kernel proof did not execute the required enforcement test" >&2
  exit 1
}
...
cat > "${REPORT}" <<EOF
{
  ...
  "bpf": {
    ...
    "manifest_sha256": "${BPF_MANIFEST_SHA}",
    "lsm_object_sha256": "${LSM_OBJECT_SHA}",
    "connect_object_sha256": "${CONNECT_OBJECT_SHA}"
  },
  "evidence": {
    "proof_log_sha256": "${PROOF_LOG_SHA}"
  }
}
EOF
