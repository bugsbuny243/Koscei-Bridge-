...
const retiredPage=fs.readFileSync(path.join(root,'public','kosch-access.html'),'utf8');
...
requireText(account,'Account & SaaS Access','account SaaS identity');
requireText(account,'Identity is one layer.','identity/access separation');
requireText(account,'Billing authorization is another.','billing/access separation');
requireText(account,'id="plan"','plan field');
requireText(account,'id="outputCapacity"','output capacity field');
requireText(account,'active Enterprise SaaS entitlement','Enterprise API key boundary');
requireText(account,'/js/customer-access-v2.js?v=3','account access controller');
requireText(account,'/pricing','pricing route');
forbid(account,/KOSCH Access|TOKEN ACCESS EVIDENCE|Official mint snapshot|LIVE TOKEN POLICY/i,'holder authorization UI');

requireText(retiredPage,'KOSCH holder access<br>has been retired.','holder page tombstone');
requireText(retiredPage,'grants no product plan, API permission, discount, evidence weight, security privilege, or verdict authority','token authority retirement');
requireText(retiredPage,'/pricing','replacement SaaS pricing route');
forbid(retiredPage,/customer-access-v2\.js/,'retired holder page runtime access controller');
...
console.log('customer SaaS access contract: ok');
