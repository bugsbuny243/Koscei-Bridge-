...
const route='mux.HandleFunc("/api/customer/web3/transaction-state-recheck", solana(requiresDB(h, planTierAccess("professional", method("POST", h.TransactionGuardStateRecheck)))))';
if(!server.includes(route))throw new Error('customer state recheck must reuse existing handler behind Professional entitlement-only access');
...
for(const required of ["const recheckEndpoint='/api/customer/web3/transaction-state-recheck'","script.src='/js/koschei-auth.js?v=33'",'auth.apiCall(path,options)','permit_token:snapshot.permitToken','state_witness:snapshot.stateWitness',"credentials:'same-origin'","const action=String(data?.action||'').toLowerCase();","const permitVersion=String(permit?.version||'');","const stateBoundPermit=permitVersion==='koschei-transaction-guard-permit-v2'||permitVersion==='koschei-transaction-guard-permit-v3';","if(action==='allow'&&stateBoundPermit&&data?.enforcement_permit_issued===true","const safe=response.ok&&data?.ok===true&&data?.safe_to_proceed===true;",'function working(){\n  clearPendingRecheck();','if(pendingRecheck===snapshot)clearPendingRecheck();',"window.addEventListener('pagehide',clearPendingRecheck)"])if(!ui.includes(required))throw new Error('customer state recheck UI contract missing: '+required);
