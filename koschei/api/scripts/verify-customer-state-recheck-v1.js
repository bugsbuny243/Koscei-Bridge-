...
for(const required of ["const recheckEndpoint='/api/customer/web3/transaction-state-recheck'",'permit_token:snapshot.permitToken','state_witness:snapshot.stateWitness',"credentials:'same-origin'","const action=String(data?.action||'').toLowerCase();","if(action==='allow'&&data?.enforcement_permit_issued===true","const safe=response.ok&&data?.ok===true&&data?.safe_to_proceed===true;",'function working(){\n  clearPendingRecheck();','clearPendingRecheck();',"window.addEventListener('pagehide',clearPendingRecheck)"])if(!ui.includes(required))throw new Error('customer state recheck UI contract missing: '+required);
