(()=>{
  ...
  function installCustomerNavigation(){
    ...
    if(nav){
      nav.classList.add('customer-nav-v3');nav.innerHTML='';
      for(const [href,label] of topLinks){const a=document.createElement('a');a.href=href;a.textContent=label;if(navActive(href,current))a.setAttribute('aria-current','page');nav.appendChild(a);}
    }
  ...
  function installReveal(){
    ...
    const observer=new IntersectionObserver(entries=>entries.forEach(entry=>{if(!entry.isIntersecting)return;entry.target.classList.add('is-visible');observer.unobserve(entry.target);}),{rootMargin:'0px 0px -8% 0px',threshold:.08});
    nodes.forEach(node=>observer.observe(node));
  }

  async function hydrateHealth(){
    const indicators=[...document.querySelectorAll('[data-koschei-live]')];if(!indicators.length)return;
    const controller=new AbortController();const timer=window.setTimeout(()=>controller.abort('koschei_health_timeout'),HEALTH_TIMEOUT_MS);
    try{
      const response=await fetch('/health',{cache:'no-store',credentials:'same-origin',signal:controller.signal});const data=await response.json().catch(()=>({}));
      if(!response.ok)throw new Error(data.details||data.error||`HTTP ${response.status}`);
      const arvis=data.arvis||{},status=String(arvis.pipeline_status||arvis.status||data.status||'ready').toLowerCase();
      const isLive=['ready','healthy','live','connected','ok','manual'].some(value=>status.includes(value));
      indicators.forEach(node=>{node.classList.toggle('is-live',isLive);node.dataset.koscheiDependencyState=isLive?'ready':'degraded';node.textContent=isLive?'ARVIS production pipeline ready':'DEGRADED · production pipeline could not be verified';});
    }catch(error){
      indicators.forEach(node=>{node.textContent='DEGRADED · evidence service unavailable';node.title=error?.name==='AbortError'?`Health check did not respond within ${HEALTH_TIMEOUT_MS/1000} seconds`:String(error?.message||'dependency error');node.dataset.koscheiDependencyState='degraded';node.classList.remove('is-live');});
  ...
  function installCurrentNav(){const current=cleanPath(location.pathname);document.querySelectorAll('.koschei-global-nav a,.nav a').forEach(link=>{const path=cleanPath(new URL(link.href,location.origin).pathname);if(path===current)link.setAttribute('aria-current','page');});}
  ...
  function loadPageEnhancements(){
    ...
    if(current==='/dashboard'){
      loadStyle('/css/customer-workspace-plans-v3.css?v=1','workspace-plans-v3');
      loadEnhancement('/js/customer-workspace-plans-v3.js?v=1','workspace-plans-v3');
    }
  }
