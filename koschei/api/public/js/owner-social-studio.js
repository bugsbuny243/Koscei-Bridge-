...
function aiPrompt(payload){
  ...
  return `OWNER_EVIDENCE_STUDIO_REQUEST\nPlatform: ${spec.label}\nReturn ONLY one JSON object with exactly these keys: title, caption, description, hashtags, mentions, voiceover, hook, cta. hashtags and mentions must be arrays of strings. Write public-facing content in English. Use ONLY the supplied ARVIS scan facts when source_type is arvis. Use ONLY the supplied Koschei result facts for every other source. Never invent a wallet, transaction, block/slot, price, score, crime, identity, partnership, endorsement or certainty. Preserve PASS/FAIL/BLOCK/WITHHOLD/INSUFFICIENT_EVIDENCE semantics exactly when present. Separate observation from assessment. If evidence is incomplete, say so. Capability is not intent. Do not promise investment returns. Keep caption platform-native and concise. Voiceover should be 35-55 seconds. For YouTube include a searchable short title and fuller description. For X keep caption compact.\nKOSCHEI_RESULT=${scan}`.slice(0,7600);
}
