...
function displayAction(action,risk,level){return action==='allow'&&(risk===null||level!=='low')?'withhold':action;}
...
function tone(action,level){if(action==='block'||level==='critical'||level==='high')return'bad';if(action==='allow')return'good';return'warn';}
...
form?.addEventListener('submit',async event=>{
  event.preventDefault();const transaction=String($('transaction')?.value||'').trim();if(!transaction){$('transaction')?.focus();return;}
  ...
  try{
    ...
    const rawAction=actionOf(data?.action),level=riskLevel(data?.risk_level),risk=numeric(data?.risk_index),action=displayAction(rawAction,risk,level),units=numeric(data?.simulation?.units_consumed);
    empty.hidden=true;result.hidden=false;
    $('action').textContent=actionLabel(action);$('action').className=`safety-chip safety-action ${tone(action,level)}`;
    ...
    if(action==='withhold')$('warning').textContent=rawAction==='allow'?'Signing guidance was withheld because the response did not contain the complete low-risk evidence required for a clear display.':'Decision authority was not present in the response. Signing guidance is withheld.';
