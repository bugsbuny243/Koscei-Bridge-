...
func canonicalityRPCServer(t *testing.T, chainID, blockHash, head string) *httptest.Server {
	...
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case "eth_getBlockByNumber":
			result = map[string]any{"hash": blockHash}
...
func TestRPCForkCanonicalityVerifierPassesCanonicalFreshBlock(t *testing.T) {
	...
	server := canonicalityRPCServer(t, "0x1", hash, "0x69")
	defer server.Close()
	v := RPCForkCanonicalityVerifier{RPCURL: server.URL, MaxHeadLag: 5}
	if err := v.VerifyCanonical(context.Background(), 1, 100, hash); err != nil {
		t.Fatal(err)
	}
}
...
func TestRPCForkCanonicalityVerifierRejectsMissingFreshnessPolicy(t *testing.T) {
	hash := "0x" + strings.Repeat("a", 64)
	server := canonicalityRPCServer(t, "0x1", hash, "0x64")
	defer server.Close()
	v := RPCForkCanonicalityVerifier{RPCURL: server.URL, MaxHeadLag: 0}
	if err := v.VerifyCanonical(context.Background(), 1, 100, hash); err == nil {
		t.Fatal("zero MaxHeadLag disabled freshness gate")
	}
}

func TestRPCForkCanonicalityVerifierRejectsReorgedBlock(t *testing.T) {
	hash := "0x" + strings.Repeat("a", 64)
	server := canonicalityRPCServer(t, "0x1", "0x"+strings.Repeat("b", 64), "0x69")
	defer server.Close()
	v := RPCForkCanonicalityVerifier{RPCURL: server.URL, MaxHeadLag: 5}
	if err := v.VerifyCanonical(context.Background(), 1, 100, hash); err == nil {
		t.Fatal("reorged block accepted")
	}
}

func TestRPCForkCanonicalityVerifierRejectsStaleReference(t *testing.T) {
	hash := "0x" + strings.Repeat("a", 64)
	server := canonicalityRPCServer(t, "0x1", hash, "0x70")
	defer server.Close()
	v := RPCForkCanonicalityVerifier{RPCURL: server.URL, MaxHeadLag: 5}
	if err := v.VerifyCanonical(context.Background(), 1, 100, hash); err == nil {
		t.Fatal("stale reference block accepted")
	}
}
