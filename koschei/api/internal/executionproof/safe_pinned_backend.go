...
//
// Implementations must not expose production signing or forwarding authority.
...
type SafeSimulationResult struct {
	...
	CallPathFullyObserved bool
}

// PinnedSafeBackend composes a concrete isolated Safe simulation engine into
// the evidence contract consumed by SafeIsolatedRunner. It verifies the pinned
// block and runner identity before executing, snapshots Safe authority before
// execution, and requires a fully shaped post-execution result.
...
func (b PinnedSafeBackend) ExecuteSafe(ctx context.Context, input matrixcontainment.CellInput, tx SafeTransaction) (SafeExecutionEvidence, error) {
	...
	if err := ctx.Err(); err != nil {
		return SafeExecutionEvidence{}, err
	}
	...
	if err != nil {
		return SafeExecutionEvidence{}, fmt.Errorf("resolve pinned block: %w", err)
	}
	if !equalHex32(pinnedHash, input.BlockHash) {
		return SafeExecutionEvidence{}, fmt.Errorf("pinned block identity mismatch")
	}

	runnerSHA, err := b.Engine.RunnerSHA256(ctx)
	if err != nil {
		return SafeExecutionEvidence{}, fmt.Errorf("resolve runner identity: %w", err)
	}
	...
	if err != nil {
		return SafeExecutionEvidence{}, fmt.Errorf("snapshot Safe pre-state: %w", err)
	}
	if !validAuthoritySnapshot(before) || !validSHA256Text(preStateSHA) {
		return SafeExecutionEvidence{}, fmt.Errorf("invalid Safe pre-state evidence")
	}

	result, err := b.Engine.ExecuteExactSafe(ctx, input, tx)
	if err != nil {
		return SafeExecutionEvidence{}, fmt.Errorf("execute exact Safe action: %w", err)
	}
	if err := ctx.Err(); err != nil {
		return SafeExecutionEvidence{}, err
	}
	...
	for _, movement := range result.AssetMovements {
		if !validAssetMovement(movement) {
			return SafeExecutionEvidence{}, fmt.Errorf("invalid Safe asset movement evidence")
		}
	}
	...
	return SafeExecutionEvidence{
		...
		BlockHash:             strings.TrimSpace(pinnedHash),
		...
		Before:                before,
		After:                 result.PostAuthority,
		AssetMovements:        append([]SafeAssetMovement(nil), result.AssetMovements...),
		CallPathFullyObserved: result.CallPathFullyObserved,
	}, nil
}
