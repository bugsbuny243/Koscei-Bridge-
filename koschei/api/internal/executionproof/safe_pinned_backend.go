...
type PinnedSafeBackend struct { Engine SafeSimulationEngine; Accessor string }
...
func (b PinnedSafeBackend) ExecuteSafe(ctx context.Context, input executioncontainment.CellInput, tx SafeTransaction) (SafeExecutionEvidence, error) {
	...
	if tx.ChainID != input.ChainID || !strings.EqualFold(normalizeAddress(tx.To), normalizeAddress(input.Target)) { return SafeExecutionEvidence{}, fmt.Errorf("Safe transaction/input mismatch") }
	...
	if !validSHA256Text(runnerSHA) || !strings.EqualFold(strings.TrimPrefix(strings.TrimSpace(runnerSHA), "0x"), strings.TrimPrefix(strings.TrimSpace(input.ApprovedRunnerSHA256), "0x")) { return SafeExecutionEvidence{}, fmt.Errorf("runner identity mismatch") }
	...
	if err := ctx.Err(); err != nil { return SafeExecutionEvidence{}, err }
	if !validAuthoritySnapshot(result.PostAuthority) || !validSHA256Text(result.PostStateSHA256) || !validSHA256Text(result.EffectSetSHA256) { return SafeExecutionEvidence{}, fmt.Errorf("invalid Safe post-state evidence") }
	for _, movement := range result.AssetMovements { if !validAssetMovement(movement) { return SafeExecutionEvidence{}, fmt.Errorf("invalid Safe asset movement evidence") } }
	if !(SafeAccessorSemanticsVerifier{Accessor: b.Accessor}).Verify(tx, result.Trace) { return SafeExecutionEvidence{}, fmt.Errorf("invalid Safe accessor execution semantics") }
	return SafeExecutionEvidence{ChainID: input.ChainID, BlockNumber: input.BlockNumber, BlockHash: strings.TrimSpace(pinnedHash), RunnerSHA256: strings.TrimPrefix(strings.TrimSpace(runnerSHA), "0x"), PreStateSHA256: strings.TrimPrefix(strings.TrimSpace(preStateSHA), "0x"), PostStateSHA256: strings.TrimPrefix(strings.TrimSpace(result.PostStateSHA256), "0x"), EffectSetSHA256: strings.TrimPrefix(strings.TrimSpace(result.EffectSetSHA256), "0x"), Before: before, After: result.PostAuthority, AssetMovements: append([]SafeAssetMovement(nil), result.AssetMovements...), Trace: result.Trace}, nil
}
