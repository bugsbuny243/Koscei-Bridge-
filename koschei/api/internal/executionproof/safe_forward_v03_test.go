...
func safeForkFixture(t *testing.T) (SafeForwardRequest, Proof, VerifiedForkRequest, VerifiedForkBackendResult) {
	...
	if !ok { t.Fatal("safe fork request did not prepare") }
...
func TestVerifyForkAndForwardSafeTransactionBlocksReorgOrStaleState(t *testing.T) {
	...
	if forwarder.calls != 0 { t.Fatalf("stale/reorg state reached forwarder: calls=%d",forwarder.calls) }
}
...
func TestValidVerifiedForkReceiptRejectsTamperedOuterDigest(t *testing.T) {
	...
	if ValidVerifiedForkReceipt(receipt) { t.Fatal("tampered execution transaction hash remained valid") }
}
