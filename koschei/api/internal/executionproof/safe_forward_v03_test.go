...
func (f fixedCanonicalityVerifier) VerifyCanonical(context.Context, uint64, uint64, string) error {
	return f.err
}

type fixedSafeAwareVerifiedForkBackend struct{ fixedVerifiedForkBackend }

func (fixedSafeAwareVerifiedForkBackend) SafeExecutionModel() string {
	return ExecutionModelSafeSimulateTxV1
}
...
func safeForkFixture(t *testing.T) (SafeForwardRequest, Proof, VerifiedForkRequest, VerifiedForkBackendResult) {
	...
	forkReq.Payload = EVMPayload{From: req.Transaction.Safe, To: req.Transaction.To, ValueHex: "0x0", DataHex: "0x"}
...
func TestVerifyForkAndForwardSafeTransactionRejectsGenericDirectCallBackend(t *testing.T) {
	...
	if forwarder.calls != 0 {
		t.Fatalf("generic direct-call backend reached Safe forwarder: calls=%d", forwarder.calls)
	}
}
...
func TestVerifyForkAndForwardSafeTransactionRequiresMatchingVerifiedExecution(t *testing.T) {
	...
	if receipt.Execution.ExecutionModel != ExecutionModelSafeSimulateTxV1 {
		t.Fatalf("execution model=%q", receipt.Execution.ExecutionModel)
	}
...
func TestVerifyForkAndForwardSafeTransactionBlocksReorgOrStaleState(t *testing.T) {
	...
	if !ValidVerifiedForkReceipt(receipt) {
		t.Fatal("expected verified receipt before canonicality block")
	}
	assertSigningBlockedFor(t, decision, ReasonForkStateStale)
	if forwarder.calls != 0 {
		t.Fatalf("stale/reorg state reached forwarder: calls=%d", forwarder.calls)
	}
}
...
func TestVerifyForkAndForwardSafeTransactionRequiresCanonicalityVerifier(t *testing.T) {
	...
	assertSigningBlockedFor(t, decision, ReasonForkExecutionRequired)
	if forwarder.calls != 0 {
		t.Fatalf("nil canonicality verifier reached forwarder: calls=%d", forwarder.calls)
	}
}
