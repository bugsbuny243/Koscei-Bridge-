...
func TestSafeIsolatedRunnerVerifiedEvidenceCanRelease(t *testing.T) {
	...
	if receipt.Decision != matrixcontainment.DecisionRelease { t.Fatalf("decision=%s reasons=%v", receipt.Decision, receipt.Reasons) }
}
...
func TestSafeIsolatedRunnerContainsAuthorityChange(t *testing.T) {
	...
	receipt, err := matrixcontainment.EvaluateWithRunner(context.Background(), input, action, runner)
	if err != nil { t.Fatal(err) }
	if receipt.Decision != matrixcontainment.DecisionContain { t.Fatalf("decision=%s reasons=%v", receipt.Decision, receipt.Reasons) }
}
...
func TestSafeIsolatedRunnerRejectsTruncatedTraceAsUnavailable(t *testing.T) {
	...
	receipt, err := matrixcontainment.EvaluateWithRunner(context.Background(), input, action, runner)
	if err != nil { t.Fatal(err) }
	if receipt.Decision != matrixcontainment.DecisionUnavailable { t.Fatalf("decision=%s reasons=%v", receipt.Decision, receipt.Reasons) }
}
