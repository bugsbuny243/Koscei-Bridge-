...
func (v RPCForkCanonicalityVerifier) VerifyCanonical(ctx context.Context, chainID uint64, referenceBlock uint64, referenceHash string) error {
	...
	if err != nil || observedChain != chainID { return errors.New("canonical chain id mismatch") }
	observedHash, err := client.blockHash(ctx, referenceBlock)
	if err != nil || !equalHex32(observedHash, referenceHash) { return errors.New("reference block is no longer canonical") }
	var headHex string
	if err := client.call(ctx,"eth_blockNumber",[]any{},&headHex); err != nil { return fmt.Errorf("read canonical head: %w",err) }
