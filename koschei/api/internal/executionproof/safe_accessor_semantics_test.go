...
func validSafeAccessorTraceFixture(t *testing.T, tx SafeTransaction, accessor string) SafeTraceEvidence {
	...
	if !ok { t.Fatal("failed to encode accessor simulate calldata") }
	kind := "call"
	if tx.Operation == 1 { kind = "delegatecall" }
...
func TestSafeAccessorSemanticsVerifierAcceptsBoundAccessorPath(t *testing.T) {
	...
	if !(SafeAccessorSemanticsVerifier{Accessor: testSafeAccessor}).Verify(tx, trace) { t.Fatal("valid Safe accessor path rejected") }
}

func TestSafeAccessorSemanticsVerifierRejectsGenericDirectCall(t *testing.T) {
	tx := validSafeForwardRequest().Transaction
	trace := validSafeTraceFixture(tx.Safe, tx.To)
	if (SafeAccessorSemanticsVerifier{Accessor: testSafeAccessor}).Verify(tx, trace) { t.Fatal("generic direct call must not satisfy Safe semantics") }
}

func TestSafeAccessorSemanticsVerifierRejectsWrongAccessor(t *testing.T) {
	tx := validSafeForwardRequest().Transaction
	trace := validSafeAccessorTraceFixture(t, tx, testSafeAccessor)
	if (SafeAccessorSemanticsVerifier{Accessor: "0x5555555555555555555555555555555555555555"}).Verify(tx, trace) { t.Fatal("wrong accessor identity accepted") }
}
...
func TestSafeAccessorSemanticsVerifierRejectsMutatedTransaction(t *testing.T) {
	...
	if (SafeAccessorSemanticsVerifier{Accessor: testSafeAccessor}).Verify(tx, trace) { t.Fatal("trace for another Safe transaction accepted") }
}

func TestSafeAccessorCalldataDigestChangesWithOperation(t *testing.T) {
	tx := validSafeForwardRequest().Transaction
	first, ok := safeAccessorSimulateCalldataSHA256(tx)
	if !ok { t.Fatal("encode call transaction") }
	tx.Operation = 1
	second, ok := safeAccessorSimulateCalldataSHA256(tx)
	if !ok { t.Fatal("encode delegatecall transaction") }
	if first == second { t.Fatal("operation mutation did not change accessor calldata identity") }
}
