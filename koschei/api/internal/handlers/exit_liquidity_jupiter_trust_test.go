...
func TestJupiterAPIKeyForQuoteEndpointRejectsLookalikeHost(t *testing.T) {
	t.Setenv("JUPITER_API_KEY", "secret-jupiter-key")
	...
	if got := jupiterAPIKeyForQuoteEndpoint("https://api.jup.ag/swap/v1/quote"); got != "secret-jupiter-key" {
...
func TestCollectExitLiquiditySimulationFailsBeforeHTTPWhenOfficialJupiterKeyMissing(t *testing.T) {
	...
	result := collectExitLiquiditySimulation(
		...
		"solana-mainnet",
		"Mint111",
		services.TokenMarketSnapshot{PriceUSD: 2},
		services.JupiterMarketContext{},
	)
	if called.Load() {
