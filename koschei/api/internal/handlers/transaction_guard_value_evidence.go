...
func buildTransactionGuardValueEvidence(transaction, wallet string, decoded transactionGuardDecodedTransaction, cpi transactionGuardCPIFlowAnalysis) transactionGuardValueEvidence {
	...
	appendSOL := func(origin, kind, source, destination, raw string, walletOrigin bool) {
		amount, ok := transactionGuardValuePositiveInteger(raw)
		if !ok {
			out.Complete = false
			out.Limitations = append(out.Limitations, "A decoded SOL movement had an invalid lamport amount and was not included in aggregate totals.")
			return
		}
		movement := transactionGuardValueSOLMovement{
			Origin: origin, Kind: kind, Source: strings.TrimSpace(source), Destination: strings.TrimSpace(destination),
			Lamports: amount.String(), WalletOrigin: walletOrigin,
		}
		out.SOLMovements = append(out.SOLMovements, movement)
	...
	for _, movement := range decoded.SOLTransfers {
		appendSOL("outer_instruction", movement.Kind, movement.Source, movement.Recipient, movement.Lamports, wallet != "" && strings.EqualFold(strings.TrimSpace(movement.Source), wallet))
	}
	...
	mintByTokenAccount := transactionGuardValueTokenAccountMints(decoded.AutomaticBalance.Accounts)
	ownerByTokenAccount := transactionGuardValueTokenAccountOwners(decoded.AutomaticBalance.Accounts)
	...
	appendToken := func(origin, kind, programID, source, destination, mint, raw string, decimals *int, walletOrigin bool) {
		...
		if !ok {
			out.Complete = false
			out.Limitations = append(out.Limitations, "A decoded token movement had an invalid raw amount and was not included in aggregate totals.")
			return
		}
		mint = strings.TrimSpace(mint)
		if mint == "" {
			mint = firstNonEmptyString(mintByTokenAccount[strings.TrimSpace(source)], mintByTokenAccount[strings.TrimSpace(destination)])
		}
		movement := transactionGuardValueTokenMovement{
			Origin: origin, Kind: kind, ProgramID: strings.TrimSpace(programID), Source: strings.TrimSpace(source), Destination: strings.TrimSpace(destination),
			Mint: mint, AmountRaw: amount.String(), Decimals: decimals, WalletOrigin: walletOrigin,
		}
		out.TokenMovements = append(out.TokenMovements, movement)
		if mint == "" {
		...
		if aggregate == nil {
			aggregate = &transactionGuardValueTokenAggregateAccumulator{
				transfer: big.NewInt(0), walletTransfer: big.NewInt(0), burn: big.NewInt(0), walletBurn: big.NewInt(0), decimalsConsistent: true,
			}
	...
	for _, operation := range decoded.TokenOperations {
		...
		walletOrigin := wallet != "" && (strings.EqualFold(strings.TrimSpace(operation.Authority), wallet) || strings.EqualFold(strings.TrimSpace(operation.Source), wallet) || strings.EqualFold(strings.TrimSpace(operation.Account), wallet))
		if !walletOrigin && wallet != "" {
			owner := firstNonEmptyString(ownerByTokenAccount[strings.TrimSpace(operation.Source)], ownerByTokenAccount[strings.TrimSpace(operation.Account)])
			walletOrigin = strings.EqualFold(owner, wallet)
		}
		appendToken("outer_instruction", operation.Kind, operation.ProgramID, firstNonEmptyString(operation.Source, operation.Account), operation.Destination, operation.Mint, operation.AmountRaw, operation.Decimals, walletOrigin)
	}
	for _, movement := range cpi.AssetMovements {
		if !movement.InnerOnly || !strings.EqualFold(strings.TrimSpace(movement.AssetType), "token") {
			continue
		}
		appendToken("inner_instruction", movement.Kind, movement.ProgramID, movement.Source, movement.Destination, movement.Mint, movement.AmountRaw, movement.Decimals, movement.WalletOrigin)
	}
	...
	for _, mint := range mints {
		...
		out.TokenAggregates = append(out.TokenAggregates, transactionGuardValueTokenAggregate{
			Mint: mint, TransferRaw: aggregate.transfer.String(), WalletOriginTransferRaw: aggregate.walletTransfer.String(),
			BurnRaw: aggregate.burn.String(), WalletOriginBurnRaw: aggregate.walletBurn.String(), MovementCount: aggregate.movementCount,
			WalletOriginMovementCount: aggregate.walletMovementCount, Decimals: aggregate.decimals, DecimalsConsistent: aggregate.decimalsConsistent,
		})
	}

	out.Complete = out.DecodeComplete && out.CPIComplete && out.AutomaticBalanceComplete
...
func transactionGuardValuePositiveInteger(raw string) (*big.Int, bool) {
	value := new(big.Int)
