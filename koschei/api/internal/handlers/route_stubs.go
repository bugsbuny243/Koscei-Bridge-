...
func (h *Handler) memberEmailPasswordProxy(w http.ResponseWriter, r *http.Request, neonPath string) {
	...
	if jwt == "" && neonPath == "/sign-up/email" {
		fallback, fallbackConfigured := h.callMemberNeonEmailAuth(r, memberEmailAuthRequest{
			Email:       request.Email,
	...
	if jwt == "" {
		writeJSON(w, http.StatusBadGateway, map[string]string{
			"error":   "auth_session_missing",
			"message": "The authentication provider did not return a session token. Please try again.",
		})
		return
	}
	...
	if err != nil {
		writeJSON(w, http.StatusUnauthorized, map[string]string{
			"error":   "invalid_token",
			"message": "The authentication response could not be verified.",
		})
		return
	}
	...
	if err != nil {
		writeJSON(w, http.StatusServiceUnavailable, map[string]string{
			...
			"message": "The authenticated account could not be provisioned.",
		})
		return
	}
...
func (h *Handler) callMemberNeonEmailAuth(r *http.Request, request memberEmailAuthRequest, neonPath string) (memberNeonAuthResult, bool) {
	...
	payload := map[string]string{
		...
		"password":    request.Password,
		"callbackURL": memberAbsoluteAuthCallbackURL(r, request.CallbackURL),
	}
	...
	if err != nil {
		return memberNeonAuthResult{
			StatusCode: http.StatusInternalServerError,
			Data:       map[string]any{"error": "auth_request_failed"},
		}, true
	}
	...
	response, err := (&http.Client{Timeout: 10 * time.Second}).Do(requestToNeon)
	if err != nil {
		return memberNeonAuthResult{
			StatusCode: http.StatusBadGateway,
			Data: map[string]any{
				"error":   "neon_auth_unavailable",
	...
	if len(data) == 0 && response.StatusCode/100 != 2 {
		data = map[string]any{
			"error":   "auth_provider_failed",
			"message": http.StatusText(response.StatusCode),
		}
	}
	jwt, tokenFound := extractAuthToken(response, responseBody)
	safeAuthDebugLog(memberNeonEndpointName(neonPath), response.StatusCode, responseBody, response.Cookies(), tokenFound, false)
	return memberNeonAuthResult{
...
func (h *Handler) memberAuthSuccessProfile(r *http.Request, claims neonJWTClaims) (map[string]any, error) {
	...
	return map[string]any{
		"id":                stored.ID,
		"auth_subject":      stored.AuthSubject,
		"email":             stored.Email,
		"role":              firstNonEmpty(stored.Role, "member"),
		"plan_id":           firstNonEmpty(summary.Plan, stored.PlanID, "free"),
		"plan":              firstNonEmpty(summary.Plan, stored.PlanID, "free"),
