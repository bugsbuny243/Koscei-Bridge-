...
func TestCollectExitLiquiditySimulationQuotesFixedTiersReadOnly(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		responses := map[string]struct {
			...
			"500000000":   {"990000000", "0.01"},
			"5000000000":  {"9000000000", "0.10"},
			"50000000000": {"60000000000", "0.40"},
		}
	...
	if !result.Available || result.Status != "complete" || len(result.Tiers) != 3 {
		t.Fatalf("unexpected result: %+v", result)
	}
	if result.Tiers[0].EstimatedProceedsUSD != 990 || result.Tiers[0].ReferencePriceDropPct != 1 {
		t.Fatalf("unexpected $1k tier: %+v", result.Tiers[0])
	}
...
func TestCollectExitLiquiditySimulationReportsPartialRoutes(t *testing.T) {
	...
	result := collectExitLiquiditySimulation(context.Background(), rpc, server.Client(), "solana-mainnet", "Mint111", services.TokenMarketSnapshot{PriceUSD: 2}, services.JupiterMarketContext{})
	if result.Status != "partial" || !result.Tiers[0].Available || result.Tiers[1].Available || result.Tiers[2].Available {
...
func TestValidatedReadOnlyQuoteEndpointRejectsSwapPath(t *testing.T) {
	...
	if _, err := validatedReadOnlyQuoteEndpoint(endpoint.String()); err == nil {
		t.Fatal("swap endpoint must be rejected")
	}
}
