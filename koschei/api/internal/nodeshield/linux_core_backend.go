...
func (b *LinuxCOREBackend) LoadAndAttach(ctx context.Context,cfg BPFLoadConfig,objects []VerifiedBPFObject)(BPFLoadResult,error){
	...
	lsmSpec,err:=ebpf.LoadCollectionSpecFromReader(bytes.NewReader(lsmBytes)); if err!=nil{return BPFLoadResult{},fmt.Errorf("parse verified LSM object: %w",err)}; if err:=lsmSpec.LoadAndAssign(&s.lsm,nil);err!=nil{return BPFLoadResult{},fmt.Errorf("load LSM objects: %w",err)}
