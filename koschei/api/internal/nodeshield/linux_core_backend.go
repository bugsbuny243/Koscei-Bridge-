...
type linuxArtifactDigest struct {
	SHA256 [32]byte
}
...
func (o *linuxLSMObjects) Close() {
	if o.BprmCheck != nil { o.BprmCheck.Close() }
	if o.FileOpen != nil { o.FileOpen.Close() }
	if o.TaskFixSetuid != nil { o.TaskFixSetuid.Close() }
	if o.WorkloadGates != nil { o.WorkloadGates.Close() }
	if o.ArtifactBindings != nil { o.ArtifactBindings.Close() }
}
...
func (o *linuxConnectObjects) Close() {
	if o.Connect4 != nil { o.Connect4.Close() }
	if o.ProtectedCgroups != nil { o.ProtectedCgroups.Close() }
	if o.AllowedEndpoints4 != nil { o.AllowedEndpoints4.Close() }
}
...
func (b *LinuxCOREBackend) LoadAndAttach(ctx context.Context, cfg BPFLoadConfig, objects []BPFObjectManifest) (BPFLoadResult, error) {
	...
	cleanup := true
	defer func() {
		if cleanup { session.Close() }
	}()

	lsmSpec, err := ebpf.LoadCollectionSpec(lsmPath)
	if err != nil {
		return BPFLoadResult{}, fmt.Errorf("load LSM collection spec: %w", err)
	}
	if err := lsmSpec.LoadAndAssign(&session.lsm, nil); err != nil {
		return BPFLoadResult{}, fmt.Errorf("load LSM objects: %w", err)
	}

	connectSpec, err := ebpf.LoadCollectionSpec(connectPath)
	if err != nil {
		return BPFLoadResult{}, fmt.Errorf("load connect collection spec: %w", err)
	}
	if err := connectSpec.LoadAndAssign(&session.connect, nil); err != nil {
		return BPFLoadResult{}, fmt.Errorf("load connect objects: %w", err)
	}
	...
	for _, prog := range []*ebpf.Program{session.lsm.BprmCheck, session.lsm.FileOpen, session.lsm.TaskFixSetuid} {
		...
		if err != nil {
			return BPFLoadResult{}, fmt.Errorf("attach BPF LSM program: %w", err)
		}
	...
	if err != nil {
		return BPFLoadResult{}, fmt.Errorf("attach cgroup connect4 program: %w", err)
	}
	...
	if err != nil || len(artifactBytes) != sha256.Size {
		return BPFLoadResult{}, fmt.Errorf("decode artifact sha256")
	}
	var digest linuxArtifactDigest
	copy(digest.SHA256[:], artifactBytes)
	if err := session.lsm.ArtifactBindings.Update(cfg.CgroupID, digest, ebpf.UpdateAny); err != nil {
		return BPFLoadResult{}, fmt.Errorf("bind artifact digest map: %w", err)
	}
	var verifiedDigest linuxArtifactDigest
	if err := session.lsm.ArtifactBindings.Lookup(cfg.CgroupID, &verifiedDigest); err != nil || verifiedDigest != digest {
		return BPFLoadResult{}, fmt.Errorf("verify artifact digest binding")
	}
	...
	if err := session.lsm.WorkloadGates.Update(cfg.CgroupID, gate, ebpf.UpdateAny); err != nil {
		return BPFLoadResult{}, fmt.Errorf("initialize workload gate map: %w", err)
	}

	one := uint8(1)
	if err := session.connect.ProtectedCgroups.Update(cfg.CgroupID, one, ebpf.UpdateAny); err != nil {
		return BPFLoadResult{}, fmt.Errorf("initialize protected cgroup map: %w", err)
	}
	for _, endpoint := range cfg.AllowedIPv4 {
		...
		if err := session.connect.AllowedEndpoints4.Update(key, one, ebpf.UpdateAny); err != nil {
			return BPFLoadResult{}, fmt.Errorf("initialize endpoint allowlist: %w", err)
		}
	}

	b.mu.Lock()
	defer b.mu.Unlock()
	if old := b.sessions[cfg.WorkloadID]; old != nil {
		old.Close()
	}
...
func nodeShieldObjectPaths(objects []BPFObjectManifest) (string, string, error) {
	...
	for _, obj := range objects {
		...
		case "nodeshield_lsm":
			lsmPath = obj.Path
		case "nodeshield_connect":
			connectPath = obj.Path
		}
	}
	if lsmPath == "" || connectPath == "" {
		return "", "", fmt.Errorf("BPF manifest must contain nodeshield_lsm and nodeshield_connect objects")
	}
