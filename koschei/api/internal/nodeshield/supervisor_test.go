...
func TestSupervisorAllowsWithoutMutation(t *testing.T) {
	...
	s := Supervisor{WorkloadID:"w1", ObservedArtifactSHA256:"abc", Policy:RuntimePolicy{ArtifactSHA256:"abc", AllowedHosts:[]string{"api.example.com"}}, Audit:audit, Enforcer:enforcer}
...
func TestSupervisorFailsClosedWithoutEnforcer(t *testing.T) {
	s := Supervisor{WorkloadID:"w1", ObservedArtifactSHA256:"abc", Policy:RuntimePolicy{ArtifactSHA256:"abc", AllowedHosts:[]string{"api.example.com"}}, Audit:&fakeAudit{}}
	d, err := s.Handle(context.Background(), RuntimeEvent{Kind:EventNetworkConnect, Destination:"evil.example:443"})
...
func TestSupervisorAuditFailureStillKillsBlockingWorkload(t *testing.T) {
	...
	s := Supervisor{WorkloadID:"w1", ObservedArtifactSHA256:"abc", Policy:RuntimePolicy{ArtifactSHA256:"abc", AllowedHosts:[]string{"api.example.com"}}, Audit:&fakeAudit{err:errors.New("disk unavailable")}, Enforcer:enforcer}
	d, err := s.Handle(context.Background(), RuntimeEvent{Kind:EventNetworkConnect, Destination:"evil.example:443"})
...
func TestSupervisorDenyFailureEscalatesToKill(t *testing.T) {
	...
	s := Supervisor{WorkloadID:"w1", ObservedArtifactSHA256:"abc", Policy:RuntimePolicy{ArtifactSHA256:"abc", AllowedHosts:[]string{"api.example.com"}}, Audit:&fakeAudit{}, Enforcer:enforcer}
	d, err := s.Handle(context.Background(), RuntimeEvent{Kind:EventNetworkConnect, Destination:"evil.example:443"})
