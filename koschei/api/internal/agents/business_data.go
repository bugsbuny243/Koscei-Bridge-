...
LIMIT $2`, strings.TrimSpace(tenantID), adminLimit(limit))
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID, &item.TenantID, &item.SKU, &item.Kind, &item.Name, &item.Description, &item.PriceMinor, &item.Currency, &item.Availability, &item.Metadata, &item.UpdatedAt); err != nil { return nil, err }
...
func (s *Service) AdminUpsertCatalog(ctx context.Context, item CatalogItem) error {
	if s.db == nil { return ErrPersistenceUnavailable }
	...
	if item.TenantID == "" || item.SKU == "" || item.Name == "" { return ErrInvalidAdminTransition }
	if item.Availability != "unknown" && item.Availability != "available" && item.Availability != "unavailable" { return ErrInvalidAdminTransition }
	metadata := item.Metadata
	if len(metadata) == 0 || !json.Valid(metadata) { metadata = json.RawMessage(`{}`) }
...
func (s *Service) AdminKnowledge(ctx context.Context, tenantID string, limit int) ([]KnowledgeEntry, error) {
	if s.db == nil { return nil, ErrPersistenceUnavailable }
	rows, err := s.db.QueryContext(ctx, `
...
LIMIT $2`, strings.TrimSpace(tenantID), adminLimit(limit))
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.ID, &item.TenantID, &item.Key, &item.Title, &item.Body, &item.SourceURL, &item.UpdatedAt); err != nil { return nil, err }
		items = append(items, item)
	}
	return items, rows.Err()
}
...
func (s *Service) AdminUpsertKnowledge(ctx context.Context, item KnowledgeEntry) error {
	...
	if item.TenantID == "" || item.Key == "" || item.Title == "" || item.Body == "" { return ErrInvalidAdminTransition }
	_, err := s.db.ExecContext(ctx, `
