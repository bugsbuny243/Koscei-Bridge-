...
// DefaultPiRadarNetwork is the customer-facing Pi network. Pi mainnet is the
// production default; testnet remains an explicit opt-in evidence environment.
func DefaultPiRadarNetwork() string { return piMainnetNetwork }

// NormalizePiRadarNetwork maps user/API aliases to one canonical Pi network.
// An empty value is deliberately not treated as an explicit Pi selector; callers
// that already know the target is Pi should use DefaultPiRadarNetwork().
...
func piHorizonBaseURLForNetwork(network string) (*url.URL, error) {
	...
	// PI_HORIZON_URL existed before Pi mainnet support and historically meant
	// the testnet collector. Never reuse it for mainnet, because doing so could
	// silently collect evidence from the wrong network.
...
// collectPiHorizonSnapshotForNetwork is the network-aware Pi evidence transport.
// It preserves the bounded, read-only Horizon policy from the original testnet
// collector while selecting an explicit mainnet or testnet origin.
