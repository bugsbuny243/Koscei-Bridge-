...
// provenance, issuer-control and liquidity-history interpretations remain
// chain-bound and are layered only after the selected Horizon snapshot.
func AnalyzeArvisRadarsMultiChainContext(ctx context.Context, req SecurityRadarRequest) ArvisAnalysis {
	...
	if piTarget, ok := ParsePiRadarTarget(req.Target); ok {
		...
		analysis = enrichPiLiquidityHistoryEvidenceForNetwork(ctx, analysis, piTarget, req.Network)
