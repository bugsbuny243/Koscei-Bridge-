...
func tradePIWhatsAppReceive(w http.ResponseWriter, r *http.Request) {
	...
	for _, entry := range payload.Entry {
		for _, change := range entry.Changes {
			...
			for _, c := range change.Value.Contacts { nameByWAID[c.WAID] = strings.TrimSpace(c.Profile.Name) }
			for _, incoming := range change.Value.Messages {
				if incoming.Type != "text" || strings.TrimSpace(incoming.Text.Body) == "" || strings.TrimSpace(incoming.From) == "" { continue }
				fresh, err := tradePIAgentService.RegisterProviderEvent(r.Context(), tenantID, agents.ChannelWhatsApp, incoming.ID)
				if err != nil { continue }
				if !fresh { continue }
				receivedAt := time.Now().UTC()
				if unixValue, err := parseUnixSeconds(incoming.Timestamp); err == nil { receivedAt = unixValue }
				msg := agents.Message{TenantID: tenantID, Channel: agents.ChannelWhatsApp, ChannelChatID: incoming.From, ChannelUserID: incoming.From, DisplayName: nameByWAID[incoming.From], Text: incoming.Text.Body, ReceivedAt: receivedAt}
...
func tradePIAgentAdminCatalogUpsert(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(http.MaxBytesReader(w, r.Body, 256<<10)).Decode(&req); err != nil { http.Error(w, "invalid request", http.StatusBadRequest); return }
	item := agents.CatalogItem{TenantID:firstNonEmpty(req.TenantID,os.Getenv("TRADEPI_DEFAULT_TENANT"),"demo-automotive"),SKU:req.SKU,Kind:req.Kind,Name:req.Name,Description:req.Description,PriceMinor:req.PriceMinor,Currency:req.Currency,Availability:req.Availability,Metadata:req.Metadata}
	if err := tradePIAgentService.AdminUpsertCatalog(r.Context(), item); err != nil { http.Error(w, "catalog update rejected", http.StatusConflict); return }
...
func tradePIAgentAdminKnowledge(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil { http.Error(w, "knowledge unavailable", http.StatusServiceUnavailable); return }
	writeTradePIAgentJSON(w, map[string]any{"items":items})
}
...
func tradePIAgentAdminKnowledgeUpsert(w http.ResponseWriter, r *http.Request) {
	...
	if err := json.NewDecoder(http.MaxBytesReader(w, r.Body, 1<<20)).Decode(&req); err != nil { http.Error(w, "invalid request", http.StatusBadRequest); return }
	item := agents.KnowledgeEntry{TenantID:firstNonEmpty(req.TenantID,os.Getenv("TRADEPI_DEFAULT_TENANT"),"demo-automotive"),Key:req.Key,Title:req.Title,Body:req.Body,SourceURL:req.SourceURL}
	if err := tradePIAgentService.AdminUpsertKnowledge(r.Context(), item); err != nil { http.Error(w, "knowledge update rejected", http.StatusConflict); return }
