...
func registerDossierRoutes(mux *http.ServeMux, h *handlers.Handler) {
	...
	mux.HandleFunc("/contract-finding/", requiresDB(h, method(http.MethodGet, h.PublicContractFindingPage)))
	...
	mux.HandleFunc("/api/public/contract-findings", requiresDB(h, method(http.MethodGet, h.PublicContractFindings)))
	mux.HandleFunc("/api/public/contract-findings/", requiresDB(h, method(http.MethodGet, h.PublicContractFindingItem)))
	...
	mux.HandleFunc("/api/owner/defense/finding-publications", requiresDB(h, ownerOnly(h, method(http.MethodPost, h.OwnerDefenseFindingPublication))))
