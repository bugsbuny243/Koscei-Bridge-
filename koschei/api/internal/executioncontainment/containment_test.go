...
func validObservationFixture() Observation {
	return Observation{
		...
		ObservedBlockHash:          digestA,
		ObservedRunnerSHA256:       digestE,
		PreStateSHA256:             digestA,
		PostStateSHA256:            digestB,
		EffectSetSHA256:            digestC,
...
func TestEvaluateReleasesOnlyVerifiedSafeCell(t *testing.T) {
	...
	if receipt.Decision != DecisionRelease {
		t.Fatalf("decision = %s, want RELEASE; reasons=%v", receipt.Decision, receipt.Reasons)
	}
	if len(receipt.Reasons) != 0 {
		t.Fatalf("unexpected reasons: %v", receipt.Reasons)
	}
	if !Verify(receipt) {
		t.Fatal("fresh receipt did not verify")
	}
}

func TestEvaluateContainsMutatedPayload(t *testing.T) {
	input := validInputFixture()
	input.CandidatePayloadSHA256 = digestD
...
func TestEvaluateContainsAuthorityChange(t *testing.T) {
	...
	observation.AuthorityPreserved = false

	receipt, err := Evaluate(validInputFixture(), observation)
	if err != nil {
		t.Fatal(err)
	}
	if receipt.Decision != DecisionContain {
	...
	if !containsReason(receipt.Reasons, ReasonAuthorityChanged) {
		t.Fatalf("reasons=%v, want %s", receipt.Reasons, ReasonAuthorityChanged)
	}
}

func TestEvaluateContainsHiddenExecutionPath(t *testing.T) {
	observation := validObservationFixture()
	observation.ExecutionPathFullyObserved = false
	...
	if !containsReason(receipt.Reasons, ReasonHiddenExecutionPath) {
		t.Fatalf("reasons=%v, want %s", receipt.Reasons, ReasonHiddenExecutionPath)
	}
}

func TestEvaluateUnavailableFailsClosed(t *testing.T) {
	observation := Observation{BackendAvailable: false}
	receipt, err := Evaluate(validInputFixture(), observation)
	if err != nil {
		t.Fatal(err)
	}
	...
	if !containsReason(receipt.Reasons, ReasonBackendUnavailable) {
		t.Fatalf("reasons=%v, want %s", receipt.Reasons, ReasonBackendUnavailable)
	}
	if receipt.Decision == DecisionRelease {
		t.Fatal("unavailable backend must never release")
	}
}

func TestEvaluatePinnedStateMismatchContains(t *testing.T) {
	observation := validObservationFixture()
	observation.ObservedBlockHash = digestD
	...
	if !containsReason(receipt.Reasons, ReasonPinnedStateMismatch) {
		t.Fatalf("reasons=%v, want %s", receipt.Reasons, ReasonPinnedStateMismatch)
	}
}

func TestEvaluateRunnerIdentityMismatchContains(t *testing.T) {
	observation := validObservationFixture()
	observation.ObservedRunnerSHA256 = digestD
	...
	if !containsReason(receipt.Reasons, ReasonRunnerIdentityMismatch) {
		t.Fatalf("reasons=%v, want %s", receipt.Reasons, ReasonRunnerIdentityMismatch)
	}
}

func TestReceiptDeterministic(t *testing.T) {
	first, err := Evaluate(validInputFixture(), validObservationFixture())
	if err != nil {
		t.Fatal(err)
	}
	second, err := Evaluate(validInputFixture(), validObservationFixture())
	if err != nil {
		t.Fatal(err)
	}
