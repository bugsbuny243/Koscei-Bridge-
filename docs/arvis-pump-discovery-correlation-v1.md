...
For a source-reported PumpPortal new-token or migration event with a signature, ARVIS independently rereads that exact Solana transaction through the same Helius-preferred canonical RPC path used by Pump trade correlation.
